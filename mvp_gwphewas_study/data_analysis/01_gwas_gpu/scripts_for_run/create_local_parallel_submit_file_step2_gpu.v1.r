...
complete_rds <- NULL
out_rds <- paste(".//output/HARE_ANC_Run/SUBMIT/complete_step2.finishedstep1tasks.gpu", group, "rds", sep=".")
count = 0
submit_count = 1
for (rds in rds_files)
{
    if (count == 0){
        # write the header
	submit_file <- paste(".//output/HARE_ANC_Run/SUBMIT", paste("submit.step2.all", group, "nvlm.includesnps.parallel.gpu", submit_count, "txt", sep="."), sep="/")
	...
	x$cmd <- paste("jsrun -n1 -c1 mkdir", "-p", x$nvlm_path)
        ...
        ## copy the bgen and bgi files
        x$nvlm_bgen_col <- paste(x$nvlm_path, basename(x$bgen_col), sep="/")
	x$nvlm_bgen_index_col <- paste(x$nvlm_path, basename(x$bgen_index_col), sep="/")
        #x$cmd <- paste(x$cmd, paste("jsrun -n1 -c1 cp", x$bgen_col, x$nvlm_bgen_col), sep="; ")
        #x$cmd <- paste(x$cmd, paste("jsrun -n1 -c1 cp", x$bgen_index_col, x$nvlm_bgen_index_col), sep="; ")

	# get the include list file
        x$local_group = unlist(strsplit(x$varianceRatioFile_base, ".", fixed=T))[5]
        x$include_list_file = NULL
        x$include_path = "./data/genetic_annotations/"
	...
	x$cmd <- paste(x$cmd, paste("jsrun -n1 -c1 cp", x$include_list_file, x$nvlm_include_path), sep="; ")
